...
class Conv2d_differenciator(nn.Module):
    def __init__(
                self,
                out_channels: int,
                kernel_size: int,
    ...
    def forward(self, x):
        return self.seq(self.conv1(x[:, 1:2]-x[:, :1]))


class SharedSepConv2d(nn.Module):
    conv_1 = nn.Conv2d(
    ...
    def __init__(
        ...
        self.out_channels = out_channels
        if norm['which'] == 'group':
            num_groups0 = norm['num_groups']
            norm = {key: value for key, value in norm.items()}
            norm['num_groups'] = math.gcd(out_channels//8, num_groups0)
        math.gcd(in_channels, out_channels)
        if globalConv:
            self.conv1 = SharedSepConv2d.conv_1
            self.conv2 = SharedSepConv2d.conv_2
            self.conv3 = SharedSepConv2d.conv_3
            self.conv4 = SharedSepConv2d.conv_4
            self.conv5 = SharedSepConv2d.conv_5
            self.conv6 = SharedSepConv2d.conv_6
            self.conv7 = SharedSepConv2d.conv_7
            self.conv8 = SharedSepConv2d.conv_8
        else:
            for i in range(1, 9):
                exec(
                    f"self.conv{i} = nn.Conv2d("
                    "1, 1, kernel_size=3, stride=1, padding=1, bias=False)"
                )

        for i in range(1, 9):
            exec(
                f'self.channel_conv{i} = nn.Sequential('
                'nn.Conv2d(in_channels, out_channels//8, kernel_size=1,'
                'stride=1, padding=0, bias=bias),'
                'get_norm2d(norm, out_channels//8),'
                'activations[ACTIVATION]'
                ')'
            )

    def weight_scaling(self):
        print('SHARED SEP CONV: WEIGHT SCALING NOT IMPLEMENTED')
        weight_scaling3d(self.conv1.weight)
        weight_scaling3d(self.channel_conv[0].weight)

    def HE_init(self):
        init_he(self.conv1)
        init_he(self.channel_conv[0])
    ...
    def forward(self, x):
        ...
        n0, nx = 0, self.out_channels//8
        n1 = n0 + nx
        out[:, n0:n1, :, :] = self.channel_conv1(
            self.conv1(
...
class SharedSepConv2d_v1(nn.Module):
    nx = 8
    conv2d = nn.Conv2d(
        in_channels=1,
        out_channels=nx,
    ...
    def __init__(
        ...
        if num_maps > 0:
            self.conv1 = nn.Conv2d(
                1, num_maps, kernel_size=3, stride=1, padding=1, bias=False
            )
            self.nx = num_maps
        else:
            self.conv1 = SharedSepConv2d.conv2d
            self.nx = SharedSepConv2d.nx

        self.channel_conv = nn.Sequential(
            nn.Conv2d(
                in_channels*self.nx, out_channels, kernel_size=1,
                stride=1, padding=0, bias=bias,
    ...
    def forward(self, x):
        # out = torch.empty(
        #     (x.shape[0], self.in_channels*self.nx) + x.shape[-2:],
        #     dtype=torch.float, device=x.device
        #     )
        # conv1 = self.conv1
        # i0, nx = 0, self.nx
        ...
        #     i1 = i0 + nx
        #     out[:, i0:i1, :, :] = conv1(x[:, i:i+1])
        #     i0 = i1
        out = self.conv1(
            x.view(-1, 1, x.shape[-2], x.shape[-1])
        ).reshape(x.shape[0], -1, x.shape[-2], x.shape[-1])
        return self.channel_conv(out)
...
class DenseConv2dLayer(nn.Module):
    def __init__(
        ...
        norm_block = get_norm2d(norm, sub_channels)
        if norm is not None:
    ...
    def weight_scaling(self):
        # weight_scaling2d(self.seq[].weight)
        self.seq[self.n_ws].weight_scaling()
...
class DenseConv2dTransitionLayer(nn.Module):
    def __init__(
        ...
        norm_block = get_norm2d(norm, self.out_channels)
...
class DenseConv2dBlock(nn.Module):
    def __init__(
        ...
        self.seq = nn.Sequential(*layers)
        self.transition = DenseConv2dTransitionLayer(
...
class DICNet_Dense(nn.Module):
    ...
    def __init__(
        ...
        # self.SharedSepConvInfos = SharedSepConvInfos
