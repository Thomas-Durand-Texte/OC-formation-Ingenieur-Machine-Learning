...
% \includeonlyframes{CCL,CCL2,Merci}
% \includeonlyframes{FiltrageNaN2,fillMean,fillImputer}
...
\begin{frame}[t, label=applicationProposition]
	...
	\begin{tikzpicture}
		...
		\visible<10->{
			\begin{scope}[xshift=0.79\textwidth, yshift=0.5\textheight]
				\visible<10>{
					\node[draw, rounded corners, very thick, myOrng, text width=16mm, minimum height=16mm]
						at (7mm,-1.5mm) {} ;
				}
				\node[text centered] at (5mm,3mm) {\alert{Ciblage}:} ;
				\foreach \i/\x/\A in {0/teneur en sel/A , 1/énergie/B , 2/IG ?/C }
				{
					\node[font=\fontsize{6}{6}\selectfont, anchor=west] (\A)
						at (0,-\i*3mm) { $\bullet$ \x } ;
					% \draw[myarrow] (Recom2) |- (\A) ;
...
\subsection{Remplissage des valeurs éparses}
...
\begin{frame}[t, label=CCL]
	...
	\begin{tikzpicture}
		...
		\node[green, text centered] at (0.25\textwidth,0.8\textheight) {Faisables (à priori):} ;
		
		\begin{scope}[xshift=0.25\textwidth]
			\node[box_style00, minimum height=12mm, text width=35mm] at (0,0.65\textheight) {Régime pauvre en sel} ;
			\node[box_style00, minimum height=12mm, text width=35mm] at (0,0.5\textheight) {Gestion de poids/IMC} ;
			\node[box_style00, minimum height=12mm, text width=35mm] at (0,0.35\textheight) {Vérification de la présence d'allergènes} ;
		\end{scope}
		
		\node[red, text centered] at (0.75\textwidth,0.8\textheight) {Non faisables en l'état:} ;
		\node[box_style00, minimum height=12mm, text width=35mm] at (0.75\textwidth,0.65\textheight) {Gestion de l'indice glycémqique} ;

		\node[text width=95mm, text centered] at (0.5\textwidth,0.16\textheight) {Se rapprocher de \alert{spécialistes} du domaines\\est \alert{nécessaire} pour apporter une\\meilleure \alert{connaissance métier}} ;
...
\begin{frame}[t, label=CCL2]
	...
	\begin{tikzpicture}
		...
		\node[box_style00, text width=40mm] (A) at (0.25\textwidth,0.6\textheight) {Améliorer la gestion des NaN} ;
		
		\node[box_style00, text width=40mm] (B) at (0.25\textwidth,0.4\textheight) {déterminer le pnns groups à partir de la liste des ingrédients} ;
		\draw[myarrow] (A) -- (B) ;
		
		\node[box_style00, text width=40mm] (C) at (0.75\textwidth,0.6\textheight) {Mettre en place des moteurs de rocommandation} ;
		
		\begin{scope}[xshift=0.65\textwidth]
			\node[box_style00, text width=18mm] (D) at (0,0.4\textheight) {basé sur le cotenu} ;
			\draw[myarrow] (0,51.4mm) -- (D) ;
		\end{scope}
		
		\begin{scope}[xshift=0.85\textwidth]
			\node[box_style00, text width=18mm] (E) at (0,0.4\textheight) {filtrage collaboratif} ;
