...
class NN_Trainer():
    ...
    def set_scheduler(self, scheduler: dict):
        ...
        if name not in schedulers:
            ...
            return
        self.scheduler_info = scheduler
        self.scheduler = schedulers[name](self.optimizer,
                                          **scheduler['params'])
    ...
    def set_optimizer(self, optimizer: dict):
        ...
        self.optimizer = optimizers[name](self.model.parameters(),
                                          **optimizer['params'])
...
def AlexNet_based_architecture(n_out, greyscale: bool):
    ...
    # shape: 5 x 5
    dense8 = nn.Linear(in_features=5*5*256, out_features=1024)
    LReLU8 = nn.LeakyReLU(inplace=True)
    init_he(dense8)
    dense9 = nn.Linear(in_features=1024, out_features=1024)
    LReLU9 = nn.LeakyReLU(inplace=True)
    init_he(dense9)
    dense10 = nn.Linear(in_features=1024, out_features=n_out)
...
class separableConv2d(nn.Module):
    def __init__(self, n_input: int, n_filters: int, kernel_size: int,
        ...
        self.cl0 = nn.Conv2d(n_input, n_filters, kernel_size=kernel_size,
                             stride=stride, padding=padding,
                             groups=min(n_input, n_filters))
        init_he(self.cl0)
        self.bn0 = nn.BatchNorm2d(n_filters)

        self.cl1 = nn.Conv2d(n_filters, n_filters, kernel_size=1, stride=1,
                             padding=0)
        # init_he(self.cl1)
        self.cl1.weight.data.fill_(1.)
...
class ResidualUnit(nn.Module):
    def __init__(self, n_input: int, n_filters: int, stride: int,
                 separableConv: bool, activation: Callable, add_SEblock: bool):
        super().__init__()
        self.n_input = n_input
        self.n_filters = n_filters
        self.stride = stride
        ...
        if separableConv:
            self.cl0 = separableConv2d(n_input, n_filters, kernel_size=3,
                                       stride=stride, padding=1,
                                       activation=activation)
            self.bn0 = nn.BatchNorm2d(n_filters)
            self.cl1 = separableConv2d(n_filters, n_filters, kernel_size=3,
                                       stride=1, padding=1,
                                       activation=activation)
        else:
            self.cl0 = nn.Conv2d(n_input, n_filters, kernel_size=3,
                                 stride=stride, padding=1)
            self.bn0 = nn.BatchNorm2d(n_filters)
            self.cl1 = nn.Conv2d(n_filters, n_filters, kernel_size=3,
                                 stride=1, padding=1)
            init_he(self.cl0)
            init_he(self.cl1)
        ...
        if add_SEblock:
            self.SEblock = SEblock(n_filters)
        self.add_SEblock = add_SEblock

        # self.avgPool = nn.AvgPool2d(3, stride=stride, padding=0)
        # self.avgPool2 = nn.AvgPool2d(3, stride=1, padding=0)
        if stride == 2:
            self.cl3 = nn.Conv2d(n_input, n_filters, kernel_size=1, stride=2,
                                 padding=0)
    ...
    def forward(self, x):
        ...
        # print('out:', out.shape)
        # print('avgPool(x):', self.avgPool(x).shape)
        # print('cl3(avgPool(x)):', self.cl3(self.avgPool(x)).shape)
        # out = out + self.cl3(self.avgPool2(self.avgPool(x)))
        if self.stride == 2:
            tmp = self.cl3(x)
...
def base_transform(target_shape: tuple, greyscale: bool):
    if greyscale:
...
def plot_accuracies_vs_epochs(title: str, savename: str, results: dict,
    ...
    fig, axs = plt.subplots(nrows=3, figsize=(5, 7), sharex=True,
    ...
    for i, results_i in enumerate(results.values()):
        ...
        if results_i['metric label'] == 'Accuracy':
            metric_train *= 100.
            metric_test *= 100.
        x = np.arange(1, len(metric_train)+1)

        ax_lr.plot(x, results_i['lr'], color=colors[i])
        axs[0].plot(x, metric_test, color=colors[i], label=label)
        axs[0].plot(x, metric_train, '--', color=colors[i])
        axs[1].plot(x, results_i['loss']['test'], color=colors[i], label=label)
        axs[1].plot(x, results_i['loss']['train'], '--', color=colors[i])
        xmax = max(xmax, x[-1])
    ...
    axs[0].legend(handles=[ln1, ln2], labels=['test set', 'train set'],
                  loc='lower right')
