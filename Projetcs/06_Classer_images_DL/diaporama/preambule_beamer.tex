...
\tikzstyle{conv} = [box_style00, fill=blue, text width=45mm, minimum height=5mm]
\tikzstyle{pool} = [box_style00, fill=red, text width=35mm]
\tikzstyle{dense} = [box_style00, fill=black!70, text width=40mm, minimum height=5mm]
\tikzstyle{transition} = [box_style00, fill=myOrng, text width=40mm, minimum height=5mm]
\tikzstyle{regularisation} = [box_style00, fill=magenta, text width=35mm]
\tikzstyle{neuron} = [draw, circle, fill=blue, minimum width=5mm] 

\renewcommand<>{\SmallLayer}[2]{
	\begin{scope}[yshift=-3.2mm - (#1-1)*1.2mm]
		\fill#3[draw, fill=#2, rounded corners=0.5mm] (-20mm,-0.5mm) rectangle (20mm,0.5mm) ;
	\end{scope}
}

\definecolor{clrFactiv}{rgb}{1,1,0}


\renewcommand<>{\BatchNorm}[1]{\SmallLayer#2{#1}{cyan}}
\renewcommand<>{\Activation}[1]{\SmallLayer#2{#1}{yellow}}
...
\newcommand{\midX}{0.5\textwidth}
\newcommand{\midY}{0.5\textheight}
\newcommand{\drawBorder}{\draw[clrBorders] (1.25mm,1.25mm) rectangle (\txw-1.25mm,\txh-1.25mm) ;}
...
\newcommand{\xPageNumber}{105mm}
\newcommand{\yPageNumber}{3mm}
\tikzstyle{stylePageNumber}=[black!30, font=\small]
\newcommand{\nodePageNumber}{ \node[stylePageNumber] at (\xPageNumber, \yPageNumber) {\insertframenumber} ; }
...
\newcommand<>{\tbf}[1]{\textbf#2{#1}}
\newcommand<>{\tit}[1]{\textit#2{#1}}
\newcommand<>{\tcit}[2]{\textcolor#3{#1}{\textit#3{#2}}}

\newcommand{\chckmrk}[0]{\Large \textcolor{mygreen}{\checkmark}}

\definecolor{myOrng}{rgb}{0.9,0.5,0}
\renewcommand<>{\alert}[1]{\textcolor#2{myOrng}{\tbf{#1}}}
\renewcommand<>{\highlight}[1]{\textcolor#2{myOrng}{#1}}
\renewcommand<>{\highlight}[1]{\textcolor#2{myOrng}{#1}}
